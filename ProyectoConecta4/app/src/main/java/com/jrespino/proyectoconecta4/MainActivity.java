...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(PreferenceManager.getDefaultSharedPreferences(getApplicationContext()).getString("jugadorEmpieza","1").equals("2")){
            game.setTurno(Game.MAQUINA);
            jugarMaquina();
        }
    ...
    private void jugarMaquina() {
        ...
        int columna = game.maquinaRespondeMovimientoA(game.columnaAleatoria());
        Coordenada coordenada = new Coordenada(game.filSelect(columna),columna);
        game.colocarFicha(coordenada);
        dibujarFicha(coordenada, game.getTurno());
        if (comprobarGanador(coordenada))
            game.setEstado('F');
